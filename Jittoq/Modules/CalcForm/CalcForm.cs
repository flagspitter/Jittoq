...
{
	public partial class CalcForm : UserControl, IJittokModule
	{
		////////////////////////////////////////////////////////////////
		#region 内部で使用するクラス
		////////////////////////////////////////////////////////////////
		...
		{
			private List<string> Hist = new List<string>();
			private int Pos;
		...
		// 計算用
		private Calculator.CalculatorParser Parser = new Calculator.CalculatorParser();
		private string ErasedExpression; // for undo
		private History Hist = new History();
		
		public string IniCategory { get; } = "Calc";
		...
		// 色
		public Color ForeResult                { get; set; }
		public Color ForeResultPast            { get; set; }
		public Color BackResult                { get; set; }
		public Color ForeResultHex             { get; set; }
		public Color ForeResultHexPast         { get; set; }
		public Color BackResultHex             { get; set; }
		public Color ForeExpression            { get; set; }
		public Color BackExpression            { get; set; }
		public Color ForeExpressionDeactivated { get; set; }
		public Color BackExpressionDeactivated { get; set; }
		
		// フォント
		public Font ResultFonmt    { get => lbResult.Font;      set => lbResult.Font = value; }
		public Font ResultHexFonmt { get => lbResultHex.Font;   set => lbResultHex.Font = value; }
		...
		private double ParseAndCalc()
		{
			double ret;
			if( Parser.Parse( txtExpression.Text.Trim() ) )
			{
				ret = Parser.Answer;
			}
			else
		...
		public string ModuleName => "Calculator";
		public char   AcceleratorKey => 'C';
		...
		{
			lbResult.ForeColor      = ForeResult     = Common.LoadColorSetting( ini, IniCategory, "ForeResult",     "White" );
			lbResult.BackColor      = BackResult     = Common.LoadColorSetting( ini, IniCategory, "BackResult",     "DimGray" );
			lbResultHex.ForeColor   = ForeResultHex  = Common.LoadColorSetting( ini, IniCategory, "ForeResultHex",  "White" );
			lbResultHex.BackColor   = BackResultHex  = Common.LoadColorSetting( ini, IniCategory, "BackResultHex",  "DimGray" );
			txtExpression.ForeColor = ForeExpression = Common.LoadColorSetting( ini, IniCategory, "ForeExpression", "White" );
			txtExpression.BackColor = BackExpression = Common.LoadColorSetting( ini, IniCategory, "BackExpression", "Black" );
			
			ForeResultPast            = Common.LoadColorSetting( ini, IniCategory, "ForeResultPast",    "LightGray" );
			ForeResultHexPast         = Common.LoadColorSetting( ini, IniCategory, "ForeResultHexPast", "LightGray" );
			ForeExpressionDeactivated = Common.LoadColorSetting( ini, IniCategory, "ForeExpressionDeactivated", "White" );
			BackExpressionDeactivated = Common.LoadColorSetting( ini, IniCategory, "BackExpressionDeactivated", "DarkGray" );
			
			SetFont( lbResult, "Font_Result" );
			SetFont( lbResultHex, "Font_Hex" );
			SetFont( txtExpression, "Font_Expression" );
			
			Hist.Max = Int32.Parse( ini.Read( IniCategory, "MaxHistory", "32" ) );
			
			ini.Exec( IniCategory, "Show", "Ctrl+Alt+Enter", ',', v => Controllers.Add( v.Trim(), hk_ShowHide ) );
			
			CursorFixer.Add( txtExpression );
		...
		{
			txtExpression.ForeColor = ForeExpression;
			txtExpression.BackColor = BackExpression;
		}
		
		public void AppsDeactivated()
		{
			txtExpression.ForeColor = ForeExpressionDeactivated;
			txtExpression.BackColor = BackExpressionDeactivated;
		}
